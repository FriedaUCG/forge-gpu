 ...
 *
 * Keep this cbuffer on explicit 16-byte lanes. Vulkan's strict uniform block
 * layout rejects vec3 padding members that are not 16-byte aligned, and the
 * same layout is easy to mirror from CPU uploads and generated WGSL.
 *
 * Uniform fields:
 ...
 *   float4 reflectivity_pad (16 bytes) — x = environment map blend factor
 *
...
cbuffer FragUniforms : register(b0, space3)
{
    ...
    float4 reflectivity_pad; /* x = environment map blend [0..1]            */
};
...
float4 main(PSInput input) : SV_Target
{
    ...
    float3 blended = lerp(surface_color.rgb, env_color, reflectivity_pad.x);
