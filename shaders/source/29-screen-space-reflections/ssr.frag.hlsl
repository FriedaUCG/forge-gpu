            ...
            float2 uv       : TEXCOORD0) : SV_Target
{
    ...
    float reflectivity = wpos_tex.SampleLevel(wpos_smp, uv, 0.0).a;
    ...
    float depth    = depth_tex.SampleLevel(depth_smp, uv, 0.0).r;
    float3 view_pos = reconstruct_view_pos(uv, depth);

    /* ── Read view-space normal ────────────────────────────────────── */
    float3 view_nrm = normalize(normal_tex.SampleLevel(normal_smp, uv, 0.0).xyz);
    ...
    for (int i = 0; i < max_steps; i++)
    {
        ...
        /* Skip early steps to avoid self-intersection. Keep the branch shape
         * simple for shader translators that are sensitive to early-continue
         * control flow in this loop. */
        if (traveled >= SSR_MIN_TRAVEL)
        {
            /* Sample the depth buffer at the projected position. */
            float sample_depth = depth_tex.SampleLevel(depth_smp, sample_uv, 0.0).r;
    ...
    for (int j = 0; j < SSR_REFINE_STEPS; j++)
    {
        ...
        float  mid_depth = depth_tex.SampleLevel(depth_smp, mid_uv, 0.0).r;
    ...
    float3 hit_color = color_tex.SampleLevel(color_smp, hit_uv, 0.0).rgb;
    ...
    float3 hit_normal = normalize(normal_tex.SampleLevel(normal_smp, hit_uv, 0.0).xyz);
