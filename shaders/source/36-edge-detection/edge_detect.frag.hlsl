...
float sobel_depth(float2 uv)
{
    ...
    float tl = depth_tex.SampleLevel(depth_smp, uv + float2(-texel_size.x,  texel_size.y), 0.0).r;
    float tc = depth_tex.SampleLevel(depth_smp, uv + float2( 0.0,           texel_size.y), 0.0).r;
    float tr = depth_tex.SampleLevel(depth_smp, uv + float2( texel_size.x,  texel_size.y), 0.0).r;
    float ml = depth_tex.SampleLevel(depth_smp, uv + float2(-texel_size.x,  0.0         ), 0.0).r;
    float mr = depth_tex.SampleLevel(depth_smp, uv + float2( texel_size.x,  0.0         ), 0.0).r;
    float bl = depth_tex.SampleLevel(depth_smp, uv + float2(-texel_size.x, -texel_size.y), 0.0).r;
    float bc = depth_tex.SampleLevel(depth_smp, uv + float2( 0.0,          -texel_size.y), 0.0).r;
    float br = depth_tex.SampleLevel(depth_smp, uv + float2( texel_size.x, -texel_size.y), 0.0).r;
...
float sobel_normal(float2 uv)
{
    ...
    float3 tl = normal_tex.SampleLevel(normal_smp, uv + float2(-texel_size.x,  texel_size.y), 0.0).rgb;
    float3 tc = normal_tex.SampleLevel(normal_smp, uv + float2( 0.0,           texel_size.y), 0.0).rgb;
    float3 tr = normal_tex.SampleLevel(normal_smp, uv + float2( texel_size.x,  texel_size.y), 0.0).rgb;
    float3 ml = normal_tex.SampleLevel(normal_smp, uv + float2(-texel_size.x,  0.0         ), 0.0).rgb;
    float3 mr = normal_tex.SampleLevel(normal_smp, uv + float2( texel_size.x,  0.0         ), 0.0).rgb;
    float3 bl = normal_tex.SampleLevel(normal_smp, uv + float2(-texel_size.x, -texel_size.y), 0.0).rgb;
    float3 bc = normal_tex.SampleLevel(normal_smp, uv + float2( 0.0,          -texel_size.y), 0.0).rgb;
    float3 br = normal_tex.SampleLevel(normal_smp, uv + float2( texel_size.x, -texel_size.y), 0.0).rgb;
...
bool is_sky_region(float2 uv)
{
    ...
    float center = depth_tex.SampleLevel(depth_smp, uv, 0.0).r;
...
float4 main(PSInput input) : SV_Target0
{
    ...
    if (show_debug != 0)
    {
        if (uv.x < 0.5)
        {
            ...
            float d = depth_tex.SampleLevel(depth_smp, uv, 0.0).r;
        ...
        else
        {
            ...
            float3 n = normal_tex.SampleLevel(normal_smp, uv, 0.0).rgb;
    ...
    float3 scene_color = color_tex.SampleLevel(color_smp, uv, 0.0).rgb;
