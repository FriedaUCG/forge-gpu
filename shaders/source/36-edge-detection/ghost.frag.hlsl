 ...
 *
 * Uniform buffers:
 *   register(b0, space3) -> slot 0: ghost color and intensity. Keep this on
 *   explicit float4 lanes so Vulkan, WGSL, and CPU uploads agree without
 *   vec3 padding members.
 *
...
{
    float4 ghost_color_power;    /* xyz = RGB tint, w = Fresnel exponent     */
    float4 ghost_brightness_pad; /* x = overall intensity multiplier         */
};
...
float4 main(PSInput input) : SV_Target0
{
    ...
    float rim = pow(1.0 - facing, ghost_color_power.w);

    /* Output with premultiplied alpha for additive blending. */
    float3 color = ghost_color_power.xyz * rim * ghost_brightness_pad.x;
