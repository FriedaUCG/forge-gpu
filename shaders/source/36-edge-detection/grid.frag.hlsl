 ...
 * shadow map sampling, and MRT output.
 *
 * Renders lit grid color to Target0 and view-space normals to Target1.
 * The normal target feeds into the edge-detection post-process pass.
 *
 * Fragment samplers (space2):
 *   slot 0 -> shadow depth texture + nearest-clamp sampler
 *
 * Uniform buffers:
 *   register(b0, space3) -> slot 0: grid rendering parameters (80 bytes)
 *
...
struct PSInput
{
    ...
    float3 view_up    : TEXCOORD2;
};

struct PSOutput
{
    float4 color  : SV_Target0;  /* lit grid color                         */
    float4 normal : SV_Target1;  /* view-space normal for edge detection   */
};
...
PSOutput main(PSInput input)
{
    ...
    /* Store view-space normal directly — RGBA16F preserves the full [-1,1] range */
    output.normal = float4(normalize(input.view_up), 1.0);
