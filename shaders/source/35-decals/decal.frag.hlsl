...
cbuffer DecalFragUniforms : register(b0, space3)
{
    ...
    float  _pad0;
    float  _pad1;
    float  _pad2;                          /* strict cbuffer/WebGPU tail padding */
};
...
float4 main(float4 sv_pos : SV_Position) : SV_Target
{
    ...
    float depth = depth_tex.SampleLevel(depth_smp, screen_uv, 0.0).r;
    ...
    float4 decal_color = decal_tex.SampleLevel(decal_smp, uv, 0.0) * decal_tint;
    ...
    float3 N = normalize(normal_tex.SampleLevel(normal_smp, screen_uv, 0.0).xyz * 2.0 - 1.0);
