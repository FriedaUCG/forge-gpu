/*
 * tree_instanced_shadow.vert.hlsl - Instanced shadow pass vertex shader
 *
 * Adapted from forge-gpu common/scene/shaders/scene_instanced_shadow.vert.hlsl.
 * The SDL demo keeps it with the terrain/tree shader family and binds the same
 * colored 80-byte instance stream as the source colored-instanced scene
 * pipeline, although this shadow pass consumes only the matrix columns.
 *
 * Per-vertex attributes (slot 0, VERTEX input rate, 24-byte stride):
 *   TEXCOORD0 -> float3 position  (location 0)
 *
 * Per-instance attributes (slot 1, INSTANCE input rate, 80-byte colored stride):
