/*
 * tree_instanced_colored.frag.hlsl - Blinn-Phong lighting with per-instance color
 *
 * Adapted from forge-gpu common/scene/shaders/scene_instanced_colored.frag.hlsl.
 * The SDL demo keeps it with the terrain/tree shader family while preserving
 * the original colored-instanced lighting behavior.
 *
...
{
    float4 base_color;      /* uniform tint (multiply with per-instance color) */
...
struct PSInput
{
    ...
    float4 inst_color : TEXCOORD3;   /* per-instance color from vertex shader */
};

float4 main(PSInput input) : SV_Target0
{
    /* ── Surface color: uniform tint × per-instance color ──────────── */
    float4 albedo = base_color * input.inst_color;
