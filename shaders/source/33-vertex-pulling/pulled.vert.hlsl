...
/* Vertex data stored in the storage buffer - matches Lesson33PulledVertex.
 * Each element is 48 bytes so the layout is explicit for WGSL/std430-style
 * storage buffer alignment as well as native backends. */
struct PulledVertex
{
    float3 position;
    float _pad0;
