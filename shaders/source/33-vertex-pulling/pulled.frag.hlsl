/*
 * Fragment shader - Blinn-Phong with directional light and shadow map.
 *
 * This is the same lighting model used in previous lessons.  Vertex pulling
 * only affects how vertex data reaches the vertex shader - the fragment
 * shader works identically regardless of whether attributes came from the
 * input assembler or a storage buffer.
 *
...
/* Shadow map resolution - must match LESSON33_SHADOW_MAP_SIZE. */
...
float4 main(float4 clip_pos   : SV_Position,
             ...
             float2 uv         : TEXCOORD2,
             float4 light_clip : TEXCOORD3) : SV_Target
{
    /* -- Surface color ------------------------------------------------ */
