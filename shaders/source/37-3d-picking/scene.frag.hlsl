/*
 * Scene fragment shader — Blinn-Phong lighting with shadow map and color tint.
 *
 * Computes directional lighting using the Blinn-Phong reflection model,
 * samples a shadow map with 2x2 PCF for soft edges, and adds an optional
 * color tint.  The tint can be used to visually distinguish selected objects
 * or highlight hovered objects during 3D picking.
 *
...
/* Shadow map resolution must match the C++ lesson's shadow map size. */
...
cbuffer SceneFragUniforms : register(b0, space3)
{
    ...
    float3 tint;            /* additive color tint for selected objects   */
    float  _pad0;
};
             ...
             float4 light_clip : TEXCOORD2) : SV_Target
{
    ...
    /* ── Ambient term with tint ─────────────────────────────────────── */
    float3 total_light = albedo.rgb * ambient + tint * ambient;
