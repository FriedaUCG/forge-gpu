...
#endif
#ifndef FORGE_SHAPES_PI_D
#define FORGE_SHAPES_PI_D  3.14159265358979323846264338327950288
#endif
#ifndef FORGE_SHAPES_TAU_D
#define FORGE_SHAPES_TAU_D 6.28318530717958647692528676655900576
#endif
...
/* Compute procedural angles in double and narrow once to float. This avoids
 * extra platform drift from float libm paths while keeping the public shape
 * data format and source lesson geometry unchanged. */
static float forge_shapes__sin_to_float(double angle)
{
    return (float)SDL_sin(angle);
}

static float forge_shapes__cos_to_float(double angle)
{
    return (float)SDL_cos(angle);
}

static double forge_shapes__atan2(double y, double x)
{
    return SDL_atan2(y, x);
}

static double forge_shapes__asin(double value)
{
    return SDL_asin(value);
}
...
ForgeShape forge_shapes_sphere(int slices, int stacks)
{
    ...
    int v = 0;
    for (int row = 0; row < rows; row++) {
        double theta = FORGE_SHAPES_PI_D * (double)row / (double)stacks;
        float sin_theta = forge_shapes__sin_to_float(theta);
        float cos_theta = forge_shapes__cos_to_float(theta);

        for (int col = 0; col < cols; col++) {
            double phi = FORGE_SHAPES_TAU_D * (double)col / (double)slices;
            float sin_phi = forge_shapes__sin_to_float(phi);
            float cos_phi = forge_shapes__cos_to_float(phi);
...
ForgeShape forge_shapes_icosphere(int subdivisions)
{
    ...
    for (int i = 0; i < num_verts; i++) {
        ...
        float u = (float)(0.5 + forge_shapes__atan2(verts[i].z, verts[i].x) /
                           FORGE_SHAPES_TAU_D);
        float v_coord = (float)(0.5 + forge_shapes__asin(verts[i].y) /
                                FORGE_SHAPES_PI_D);
...
ForgeShape forge_shapes_cylinder(int slices, int stacks)
{
    ...
    for (int row = 0; row < rows; row++) {
        ...
        for (int col = 0; col < cols; col++) {
            float fu  = (float)col / (float)slices;
            double phi = FORGE_SHAPES_TAU_D * (double)col / (double)slices;
            float cp  = forge_shapes__cos_to_float(phi);
            float sp  = forge_shapes__sin_to_float(phi);
...
ForgeShape forge_shapes_cone(int slices, int stacks)
{
    ...
    double half_angle = SDL_atan2(1.0, 2.0);
    float sin_ha = forge_shapes__sin_to_float(half_angle);
    float cos_ha = forge_shapes__cos_to_float(half_angle);
                                 ...
                                 float half_height)
{
    ...
    for (int row = 0; row < cap_stacks; row++) {
        ...
        double frac = (double)row / (double)cap_stacks;
        double theta = FORGE_SHAPES_PI_D - frac * (FORGE_SHAPES_PI_D * 0.5);
        float sin_t = forge_shapes__sin_to_float(theta);
        float cos_t = forge_shapes__cos_to_float(theta);
    ...
    for (int row = 1; row <= cap_stacks; row++) {
        double frac = (double)row / (double)cap_stacks;
        double theta = (FORGE_SHAPES_PI_D * 0.5) -
                       frac * (FORGE_SHAPES_PI_D * 0.5);
        float sin_t = forge_shapes__sin_to_float(theta);
        float cos_t = forge_shapes__cos_to_float(theta);
        float y_off = half_height;
                               ...
                               float major_radius, float tube_radius)
{
    ...
    for (int row = 0; row < rows; row++) {
        float fv = (float)row / (float)stacks;
        double theta = FORGE_SHAPES_TAU_D * (double)row / (double)stacks;
        float cos_t = forge_shapes__cos_to_float(theta);
        float sin_t = forge_shapes__sin_to_float(theta);
        ...
        for (int col = 0; col < cols; col++) {
            float fu  = (float)col / (float)slices;
            double phi = FORGE_SHAPES_TAU_D * (double)col / (double)slices;
            float cos_p = forge_shapes__cos_to_float(phi);
            float sin_p = forge_shapes__sin_to_float(phi);
