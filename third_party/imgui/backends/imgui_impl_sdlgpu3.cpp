...
static void ImGui_ImplSDLGPU3_UpdateTextureInternal(ImTextureData* tex, SDL_GPUCommandBuffer* command_buffer);
...
static uint32_t ImGui_ImplSDLGPU3_AlignUploadSize(uint32_t size)
{
    return (size + 3u) & ~3u;
}
...
void ImGui_ImplSDLGPU3_PrepareDrawData(ImDrawData* draw_data, SDL_GPUCommandBuffer* command_buffer)
{
    ...
    if (draw_data->Textures != nullptr)
        for (ImTextureData* tex : *draw_data->Textures)
            if (tex->Status != ImTextureStatus_OK)
                ImGui_ImplSDLGPU3_UpdateTextureInternal(tex, command_buffer);
    ...
    uint32_t vertex_upload_size = ImGui_ImplSDLGPU3_AlignUploadSize(vertex_size);
    uint32_t index_upload_size = ImGui_ImplSDLGPU3_AlignUploadSize(index_size);
    if (fd->VertexBuffer == nullptr || fd->VertexBufferSize < vertex_upload_size)
        CreateOrResizeBuffers(&fd->VertexBuffer, &fd->VertexTransferBuffer, &fd->VertexBufferSize, vertex_upload_size, SDL_GPU_BUFFERUSAGE_VERTEX);
    if (fd->IndexBuffer == nullptr || fd->IndexBufferSize < index_upload_size)
        CreateOrResizeBuffers(&fd->IndexBuffer, &fd->IndexTransferBuffer, &fd->IndexBufferSize, index_upload_size, SDL_GPU_BUFFERUSAGE_INDEX);
    ...
    vertex_buffer_region.size = vertex_upload_size;

    SDL_GPUBufferRegion index_buffer_region = {};
    index_buffer_region.buffer = fd->IndexBuffer;
    index_buffer_region.offset = 0;
    index_buffer_region.size = index_upload_size;
...
{
    ImGui_ImplSDLGPU3_UpdateTextureInternal(tex, nullptr);
}
...
static void ImGui_ImplSDLGPU3_CreateShaders()
{
    ...
    else if (strcmp(driver, "webgpu") == 0)
    {
        ...
        vertex_shader_info.code = (const Uint8*)wgsl_vertex;
        vertex_shader_info.code_size = sizeof(wgsl_vertex) - 1;
        fragment_shader_info.format = SDL_GPU_SHADERFORMAT_WGSL;
        fragment_shader_info.code = (const Uint8*)wgsl_fragment;
        fragment_shader_info.code_size = sizeof(wgsl_fragment) - 1;
    }
#ifdef __APPLE__
    else
    {
...
void ImGui_ImplSDLGPU3_CreateDeviceObjects()
{
    ...
    if (bd->TexSamplerLinear == nullptr)
    {
        ...
        sampler_info.min_lod = strcmp(SDL_GetGPUDeviceDriver(v->Device), "webgpu") == 0 ? 0.0f : -1000.0f;
